...
var BG = new (function(){
	...
	this.draw = function(){
			try {
				// car is turning, rotate background image around center of car and redraw graphic at an angle
				ctx.save();
				ctx.translate( (canvasW / 2), (canvasH / 2) );
				ctx.rotate( (Math.PI/-180) * CAR.bearing );

				ctx.drawImage(this.image, this.X, this.Y, this.width, this.height, (canvasW / -2), (canvasH / -2), canvasW, canvasH);
	...
	this.scroll = function(){
		// HOLY COW, TRIG!
		var acRatio = Math.sin((90 - CAR.bearing) * (Math.PI/180));
		this.Y -= (acRatio * CAR.speed);
		var bcRatio = Math.cos((90 - CAR.bearing) * (Math.PI/180));
