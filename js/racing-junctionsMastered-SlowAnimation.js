...
var BG = new (function(){
	...
	this.draw = function(){
		...
		for (var i = 0; i < course.length; i++){
			...
			// road
			var roadGradient = ctx.createLinearGradient(courseCoords[i].roadX1, courseCoords[i].roadY1, courseCoords[i].roadX2, courseCoords[i].roadY2);
			...
			ctx.beginPath();
			if (i == 0){
				ctx.moveTo(courseCoords[i].roadX1, courseCoords[i].roadY1);
				ctx.lineTo(courseCoords[i].roadX2, courseCoords[i].roadY2);
				ctx.lineTo(courseCoords[i].roadEndX2, courseCoords[i].roadEndY2);
				ctx.lineTo(courseCoords[i].roadEndX1, courseCoords[i].roadEndY1);
				ctx.closePath();
				ctx.fill();
			} else	if (i < course.length - 1) {
				if (crossSide == 1){
					ctx.moveTo(courseCoords[i].startX - this.X, courseCoords[i].startY - this.Y);
					...
					ctx.lineTo(courseCoords[i].roadEndX2, courseCoords[i].roadEndY2);
					ctx.lineTo(courseCoords[i].roadEndX1, courseCoords[i].roadEndY1);
					ctx.lineTo(courseCoords[i].roadX1, courseCoords[i].roadY1);
					ctx.closePath();
					ctx.fill();
				}
			} else	if (i == course.length - 1) {
				if (crossSide == 1){
					...
					if (crossSideEnd == 1){
						...
						ctx.lineTo(courseCoords[i].endX - this.X, courseCoords[i].endY - this.Y);
						ctx.lineTo(courseCoords[i].roadEndX2, courseCoords[i].roadEndY2);
						ctx.lineTo(courseCoords[i].roadX2, courseCoords[i].roadY2);
					} else {
						ctx.lineTo(courseCoords[i].roadEndX1, courseCoords[i].roadEndY1);
