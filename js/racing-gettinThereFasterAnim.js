...
var BG = new (function(){
	...
	this.draw = function(){
		...
		for (var i = 0; i < course.length; i++){
			...
			var distToRoad = Math.sqrt(Math.pow((centerX - startX),2) + Math.pow((centerY - startY),2));
			if (distToRoad > canvasC + roadC)
				continue;
		...
		for (var i = 0; i < course.length; i++){
			...
			crossPoint = intersection( {x: roadX1, y: roadY1}, {x: roadEndX1, y: roadEndY1},
								{x: courseCoords[ii].roadX1 - this.X, y: courseCoords[ii].roadY1 - this.Y},
								{x: courseCoords[ii].roadEndX1 - this.X, y: courseCoords[ii].roadEndY1 - this.Y});
			crossSide = 1;
			otherSide = 1;
			if (!crossPoint){
				crossPoint = intersection( {x: roadX2, y: roadY2}, {x: roadEndX2, y: roadEndY2},
									{x: courseCoords[ii].roadX2 - this.X, y: courseCoords[ii].roadY2 - this.Y},
									{x: courseCoords[ii].roadEndX2 - this.X, y: courseCoords[ii].roadEndY2 - this.Y});
				crossSide = 2;
				otherSide = 2;
			}
			if (!crossPoint){
			...
			if (i == 0){
				crossPointEnd = eval(JSON.stringify(crossPoint));
				crossSideEnd = otherSide;
			}
			// *****************************************************************************************************
			...
			for (var j = 0; j <= segLength; j += (laneH * 2)){
				ctx.beginPath();
				ctx.moveTo(startX + (Math.cos(segAngleRad) * sideWay * j),
						startY + (Math.sin(segAngleRad) * sideWay * j));
				ctx.lineTo(startX + (Math.cos(segAngleRad) * sideWay * (j + laneH)),
						startY + (Math.sin(segAngleRad) * sideWay * (j + laneH)));
				ctx.stroke();				
			}
			...
			ctx.lineWidth = borderW;
			ctx.beginPath();
			var RedStartX1 = (crossSide == 1 && crossPoint) ? crossPoint[0][0] : roadX1;
			var RedStartY1 = (crossSide == 1 && crossPoint) ? crossPoint[0][1] : roadY1;
			var RedStartX2 = (crossSide == 2 && crossPoint) ? crossPoint[0][0] : roadX2;
			var RedStartY2 = (crossSide == 2 && crossPoint) ? crossPoint[0][1] : roadY2;
